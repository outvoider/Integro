...
namespace Integro
{
	...
	class ClientTds
	{
		...
		static int HandleError(DBPROCESS* dbproc, int severity, int dberr, int oserr, char* dberrstr, char* oserrstr)
		{
			stringstream s;
		...
		static int HandleMessage(DBPROCESS* dbproc, DBINT msgno, int msgstate, int severity, char* msgtext, char* srvname, char* procname, int line)
		{
	...
	public:
		static void ExecuteCommand(const string &host
			, const string &user
		...
		static void ExecuteQuery(const string &host
			, const string &user
			, const string &password
			, const string &database
			, const string &sql
			, function<void(const Mave&)> OnRow)
		{
			ClientTds client(host, user, password);
			client.ExecuteCommand(database, sql);
		...
		void FetchResults(function<void(const Mave&)> OnRow)
		{
		...
		void ExecuteCommand(const string &database, const string &sql)
		{
			dbfreebuf(dbproc);

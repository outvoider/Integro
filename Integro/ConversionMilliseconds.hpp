...
namespace Integro
{
	...
	class ConversionMilliseconds
	{
	public:
		static milliseconds FromTimeT(const time_t t)
		{
			return duration_cast<milliseconds>(system_clock::from_time_t(t).time_since_epoch());
		}

		static time_t ToTimeT(const milliseconds m)
		{
			return system_clock::to_time_t(system_clock::time_point(m));
		}

		static milliseconds FromLdapTime(const string &lt)
		{
			tm t;
		...
		static string ToLdapTime(const milliseconds m)
		{
			stringstream s;
			auto t = ToTimeT(m);
			auto gt = localtime(&t);
		...
		static milliseconds FromUtc(const string &ut)
		{
			tm t;
		...
		static string ToUtc(const milliseconds m
